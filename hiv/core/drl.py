...
class C51():
    ...
    def add_optimizer(self):
        ...
        '''
        self.loss = 0
        self.d1 = tf.log(self.distribution[0])
        self.d2 = tf.log(self.distribution[1])
        self.d3 = tf.log(self.distribution[2])
        self.d4 = tf.log(self.distribution[3])

        for i in range(self.config.nA):
            # Only train on the taken action: i.e mask = 1
            self.loss += tf.reduce_sum(self.target_distribution[i] * tf.log(self.distribution[i] + 1e-7),\
                    axis=-1) * self.target_mask[i]
        self.debug_loss = self.loss
        ...
        tf.summary.scalar('clipped_norm', tf.global_norm(grads))
    ...
    def get_target(self, x, a, r, nx, terminal, next_counts, sess):
        ...
        initial = np.zeros((batch_size, self.config.nAtoms))
        m = [initial for i in range(self.config.nA)]
        ...
        for batch in range(batch_size):
            ...
            if not terminal[batch]:
                ...
                #print("heeere for action", a[batch])
                #print(optimistic_pdf[idx] * (u-b))
    ...
    def train(self, sess, size, opt, learning_rate):
        ...
        #for i in range(4):
             # Only train on the taken action: i.e mask = 1
        #     l += np.sum(target_dist[i] * np.log(target_dist[i]),\
        #             axis=-1) * target_mask[i]
        #print("l:")
        #print(l)
        #exit(0)
        #print("Loss:")
        #loss, debug_loss, d1, d2, d3, d4 = sess.run([self.loss, self.debug_loss, self.d1, self.d2, self.d3, self.d4], feed_dict=dic)
        #print(np.sum(d1), np.sum(d2), np.sum(d3), np.sum(d4))
        #print(loss)
