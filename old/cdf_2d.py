...
def main(name, version, opt):
    ...
    for ep in range(num_episode):
        ...
        ret = 0
        while not terminal:
            a = np.argmax(c51.CVaRopt(o, counts, c=opt, alpha=alpha, N=50))
            no, r, terminal = world.step(a)
            counts[o, a] += 1
            ret += r
            c51.observe(o, a, r/world.M, no, terminal, alpha, bonus=opt/np.sqrt(counts[o, a]))
            o = no
        if ep%1000 == 0:
            np.save('c51p/'+name+'_cdf_p_%d_%d.npy'%(ep, version), c51.p)
            ...
            for ep_t in range(trial):
                ...
                ret = 0
                while not terminal:
                    a = np.argmax(c51.CVaR(o, alpha=alpha, N=50))
                    no, r, terminal = world.step(a)
                    ret += r
                    o = no
                tot_rep[ep_t] = ret
