...
def main(args, version):
    ...
    for ep in range(config.args.num_episode):
        ...
        while not terminal:
            ...
            # update
            c51.observe(o, a, r, no, terminal, alpha=args.alpha, bonus=args.opt/np.sqrt(counts[o,a]))
            # Eval
            if config.eval:
                c51_eval.observe(o, a, r, no, terminal, alpha, bonus=0.0)
